...
def load_static_files():
    ...
    # Verificar existência dos ficheiros obrigatórios na raiz
    ...
    # 3) Concatenar todos os cenários (*.txt) dentro de ./scenarios/
...
def build_faiss_index(glossary_path: str = "./Glossary.txt",
    ...
    """
    # 1) Verificar e carregar Glossary.txt
    glossary_file = Path(glossary_path)
    if not glossary_file.exists():
        raise FileNotFoundError(f"O ficheiro '{glossary_path}' não foi encontrado.")

    # TextLoader retorna uma lista com um Document (todo o conteúdo de Glossary.txt)
    glossary_docs = TextLoader(glossary_path, encoding="utf-8").load()

    # 2) Verificar e carregar todos os cenários em scenarios_folder
    ...
    # 3) Concatenar Glossary + cenários
    all_docs = glossary_docs + scenario_docs

    # 4) Partir em chunks (para dividir documentos muito longos)
    ...
    # 5) Gerar embeddings e criar índice FAISS
...
def main():
    ...
    # 4) Inicializar o chain RetrievalQA (k_retriever=3 fragmentos FAISS)
    ...
    while True:
        ...
        try:
            # RetrievalQA preenche {context} com fragmentos relevantes de Glossary+cenários
